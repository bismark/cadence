...
## Completion Notes
Implemented in compiler pipeline:
- ✅ Added `splitSpansAcrossPages()` post-pagination transform (`compiler/src/layout/split-spans-across-pages.ts`)
- ✅ Detects timed spans that appear on multiple pages via page span rects
- ✅ Splits clip timing proportionally (character-count first, rect-area fallback)
- ✅ Rewrites `page.spanRects` and `page.textRuns` span IDs to page-specific split IDs
- ✅ Recomputes `firstSpanId` / `lastSpanId` after remap
- ✅ Integrated into both `compile` and `align` flows before `assignSpansToPages`
- ✅ Added unit tests (`compiler/test/split-spans-across-pages.test.ts`)
- ✅ Verified with fixture compile (`Advanced-Accessibility-Tests...`): 1 span split, 122 mapped spans, validation passes, no timed multi-page spans remain
