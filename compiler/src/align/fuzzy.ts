/**
 * Levenshtein-based fuzzy string matching using n-gram search.
 *
 * MIT License
...
export function findNearestMatch(needle: string, haystack: string, maxDist: number) {
  let nearest: {
    start: number;
    end: number;
    dist: number;
...
function expand(subsequence: string, sequence: string, maxDist: number) {
  ...
  for (let sequenceIndex = 0; sequenceIndex < sequence.length; sequenceIndex++) {
    ...
    const needleIndexRangeEnd = Math.min(subsequenceLength, newNeedleIndexRangeEnd + 1);
...
function* levenshteinNgram(subsequence: string, sequence: string, maxDist: number) {
  const subsequenceLength = subsequence.length;
  ...
  for (
    ...
    for (const index of searchExact(
      ...
      const rightMatch = expand(
        subsequenceAfter,
        sequence.slice(index + ngramLength, index - ngramStart + subsequenceLength + maxDist),
        maxDist,
      );
      ...
      const leftMatch = expand(
        ...
        reverse(sequence, index, Math.max(0, index - ngramStart - (maxDist - distRight))),
        maxDist - distRight,
      );
